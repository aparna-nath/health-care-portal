...
namespace HealthCare_Portal.App_Code
{
    public static class Db
    {
        ...
        private static void dbUpdateEvent(string id, DateTime start, DateTime end)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["RegistrationConnectionString"].ConnectionString))
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("UPDATE [Appointments] SET [AppointmentStart] = @start, [AppointmentEnd] = @end WHERE [Appointmentid] = @id", con);
